def CLASSIFY():
    ...
    DEFAULT_LABEL_FILE = os.path.join(
        SCRIPT_PATH, '../Bengali/labels/93-common-bengali.txt'
    )
    DEFAULT_GRAPH_FILE = os.path.join(
        SCRIPT_PATH, '../Bengali/saved-model/optimized_bengali_tensorflow.pb'
    )
    def read_image(file):
    ...
    label_file='./Bengali/labels/93-common-bengali.txt'
    image='./New.jpeg'
    graph_file='./Bengali/saved-model/optimized_bengali_tensorflow.pb'
    ...
    # Get relevant nodes.
    x = graph.get_tensor_by_name('bengali-model/input:0')
    y = graph.get_tensor_by_name('bengali-model/output:0')
    keep_prob = graph.get_tensor_by_name('bengali-model/keep_prob:0')
    im = read_image(image)
